...
class Net(CNN):
    def __init__(self):
        ...
        self.affinity_layer2 = Affinity2(cfg.QCDGM.GNN_FEAT + 2)
    ...
    def forward(self, src, tgt, P_src, P_tgt, G_src, G_tgt, H_src, H_tgt, ns_src, ns_tgt, K_G, K_H, edge_src, edge_tgt, edge_feat1, edge_feat2, perm_mat, type='img'):
        ...
        emb1_normed = torch.matmul(emb1 / torch.norm(emb1, dim=2, keepdim=True), torch.abs(self.affinity_layer2.A * self.affinity_layer2.A))
        emb2_normed = torch.matmul(emb2 / torch.norm(emb2, dim=2, keepdim=True), torch.abs(self.affinity_layer2.A * self.affinity_layer2.A))
